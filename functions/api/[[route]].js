// bff/node_modules/hono/dist/http-exception.js
...
// bff/node_modules/hono/dist/router/utils.js
...
// bff/src/types.ts
var GRADE_LEVELS = ["primary", "junior", "senior", "college"];
...
function checkLen(issues, field, v, min, max) {
  const s = asTrimmed(v);
...
function checkEnum(issues, field, v, members) {
  if (typeof v !== "string" || !members.includes(v)) {
    issues.push({ field, reason: `\u987B\u4E3A ${members.join(" | ")} \u4E4B\u4E00` });
    return null;
  }
  return v;
}
function checkOptionalText(issues, field, v, max) {
...
function validateGigEntity(body) {
  const issues = [];
...
function validateGigPatch(body, current) {
  if (!isRecord(body)) {
    return { ok: false, details: [{ field: "body", reason: "\u987B\u4E3A JSON \u5BF9\u8C61" }] };
  }
...
function validateSiteConfigPatch(body) {
  if (!isRecord(body)) {
    return { ok: false, details: [{ field: "body", reason: "\u987B\u4E3A JSON \u5BF9\u8C61" }] };
  }
  const issues = [];
...
async function getGig(env, id) {
  const { data } = await client(env).query("gigs", {
    select: "*",
    filters: { id: ["eq", id] },
    limit: 1
  });
...
async function insertGig(env, value, publishedBy) {
  return client(env).insert("gigs", { ...value, published_by: publishedBy });
}
async function updateGig(env, id, patch) {
  return client(env).update("gigs", { id: ["eq", id] }, patch);
}
async function deleteGig(env, id) {
  return client(env).remove("gigs", { id: ["eq", id] });
}
async function getSiteConfig(env) {
  const { data } = await client(env).query("site_config", {
...
async function updateSiteConfig(env, patch) {
  return client(env).update("site_config", { id: ["eq", 1] }, patch);
}
async function getProfileRole(env, userId) {
...
// bff/src/middleware/adminAuth.ts
...
// bff/src/routes/gigs.ts
var UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
var STATUS_FILTERS = [...STATUSES, "all"];
...
gigs.patch("/:id", requireAdmin(), async (c) => {
  const id = c.req.param("id");
  if (!UUID_RE.test(id)) return notFound(c);
  const current = await getGig(c.env, id);
  ...
  const rows = await updateGig(c.env, id, changed);
...
siteConfig.get("/", async (c) => {
  const row = await getSiteConfig(c.env);
  if (!row) {
...
siteConfig.patch("/", requireAdmin(), async (c) => {
  ...
  const rows = await updateSiteConfig(c.env, result.value);
  return c.json({ data: rows[0] });
});
