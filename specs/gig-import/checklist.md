...
- [x] TC-IMPORT-008 用 `你好.txt` 跑全量：单子级可解析率（ok 行占比）实测 **90.8%**（196 块 / 178 ok / 18 error，含重复行口径；非重复 116 行错误 11 行），≥ 80% 达标
...
- [x] v0.1.3 解析增强回归（用户对齐颗粒度 B，import-parser.test.ts「v0.1.3 解析增强」组，BFF 全量 130 用例绿）：短标签【地址】【科目】【时间】【要求】【报酬】全部恢复 + 值内区县（长沙市开福区→kaifu）+ 每次/天/周…元→时薪 null；用户三例：例 1/例 3 ok，例 2 仅 grade 需人工（原文未写年级）
